...
def get_user_stats(user_id):
    ...
    try:
        ...
        avg_score = round(avg_score_result) if avg_score_result is not None else None
...
def essay_stats():
    ...
    try:
        ...
        cursor.execute("SELECT AVG(total_score) FROM essays WHERE total_score IS NOT NULL")
        avg_score = cursor.fetchone()[0]
        avg_score = round(avg_score, 2) if avg_score else 0
        
        cursor.execute("SELECT AVG(word_count) FROM essays WHERE word_count IS NOT NULL")
        avg_words = cursor.fetchone()[0]
        avg_words = round(avg_words, 2) if avg_words else 0
    ...
    try:
        ...
        if selected_log:
            ...
            try:
                ...
                logger.warning(f"UTF-8 解码日志文件失败，尝试 GBK 编码: {str(e)}")
                # 如果UTF-8解码失败，尝试其他编码
                try:
                    pass  # 自动修复的空块
                except Exception as e:
                    ...
                    logger.warning(f"GBK 解码日志文件也失败，使用二进制模式: {str(e2)}")
                    # 如果仍然失败，以二进制模式读取
                    with open(log_path, 'rb') as f:
                        binary_content = f.read()
                        # 将二进制内容转换为可显示的十六进制字符串
                        log_content = [f"二进制日志文件，无法以文本模式显示：{selected_log}",
                                    f"文件大小: {len(binary_content)} 字节",
                                    "请使用文本编辑器查看此文件。"]
