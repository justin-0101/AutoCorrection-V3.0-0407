...
人工智能作为现代科技发展的重要方向，已经深入到我们生活的方方面面。从智能手机上的语音助手，到自动驾驶汽车，再到医疗诊断系统，人工智能正在改变着我们的生活方式和工作方式。"""
        ...
        logger.info(f"计算得分: 内容={content_score}, 表达={language_score}, 结构={structure_score}, 书写={writing_score}, 总分={total_score}")
        ...
        # 构建评分结果 - 前端期望的格式
        # 注意：这里故意使用中文字段名，确保与前端匹配
        result = {
            # 主要得分字段
            "总分": total_score,
            "字数": len(article),
            "错别字扣分": 0,
            "内容主旨得分": content_score,
            "表达文采得分": language_score,
            "结构得分": structure_score,
            "书写得分": writing_score,
            
            # 评价和分析
            "总评": f"这是一篇结构清晰、论述合理的文章。({current_time}) {word_count_comment}",
            "综合评价": "这是一篇有一定深度的文章，作者能够清晰表达自己的观点，但在细节和例子方面还可以进一步丰富。",
            "建议改进": "建议增加更多具体例子，使用更多修辞手法来增强文章的说服力和表现力。可以适当精简一些句子，使表达更加简洁有力。",
            
            # 详细分析
            "内容分析": "文章主题明确，内容充实，但可以增加具体例子来支持论点。",
            "结构分析": "文章结构清晰，逻辑连贯，有明确的开头、正文和结尾。",
            "表达分析": "语言表达流畅，但有一些句子可以更加精炼。",
            "书写分析": "写作风格良好，但可以增加一些修辞手法来使文章更加生动。",
            
            # 优劣势
            "优点": ["观点明确", "论述清晰", "结构合理"],
            "不足": ["细节不足", "例子较少"] + word_count_weakness,
            "建议": ["建议增加具体例子", "可以使用更多修辞手法", "适当精简部分句子"],
            
            # 等级
            "等级": "A" if word_count_sufficient else "B",  # 字数不足降低等级
            ...
            "level": "A" if word_count_sufficient else "B"
        }
        ...
        # 记录最终返回的结果
        logger.info(f"最终返回结果: {json.dumps(result, ensure_ascii=False)[:500]}...")
        ...
        error_result = {
            ...
            "结构得分": "0",
            "书写得分": "0",
            "字数": str(len(article) if 'article' in locals() else 0),
            "comments": "服务器处理错误",
            "总评": f"批改过程中发生错误: {str(e)}"
        }
