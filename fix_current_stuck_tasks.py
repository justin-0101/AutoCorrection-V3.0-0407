...
def fix_inconsistent_statuses(apply_fix=False):
    """
    检查并修复不一致的状态
    
    Args:
        apply_fix: 是否应用修复（默认为False，仅检查不修改）
                ...
                if apply_fix:
                    try:
                        ...
                        with db.session() as session:
                            # 重新获取记录
                            current_essay = session.query(Essay).get(essay.id)
                            current_correction = session.query(Correction).get(correction.id)
                            
                            if not current_essay or not current_correction:
                                logger.error(f"无法找到作文 {essay.id} 或批改记录 {correction.id}")
                                continue
                                
                            # 根据任务状态决定如何修复
                            if task_status in ["PENDING", "STARTED", "RETRY"]:
                                # 任务正在处理，保持CORRECTING状态
                                current_essay.status = EssayStatus.CORRECTING
                                current_correction.status = CorrectionStatus.CORRECTING
                                logger.info(f"更新作文 {essay.id} 和批改 {correction.id} 的状态为CORRECTING")
                            elif task_status in ["SUCCESS"]:
                                # 任务已成功，但状态未更新
                                current_essay.status = EssayStatus.COMPLETED
                                current_correction.status = CorrectionStatus.COMPLETED
                                logger.info(f"更新作文 {essay.id} 和批改 {correction.id} 的状态为COMPLETED")
                            elif task_status in ["FAILURE", "REVOKED"]:
                                # 任务已失败
                                current_essay.status = EssayStatus.FAILED
                                current_correction.status = CorrectionStatus.FAILED
                                logger.info(f"更新作文 {essay.id} 和批改 {correction.id} 的状态为FAILED")
                            else:
                                # 任务状态未知或不存在，重置为待处理
                                current_essay.status = EssayStatus.PENDING
                                current_correction.status = CorrectionStatus.PENDING
                                current_correction.task_id = None
                        ...
                        try:
                            ...
                            with db.session() as session:
                                ...
                                if task_status in ["PENDING", "STARTED", "RETRY"]:
                                    ...
                                    current_essay.status = EssayStatus.FAILED
                                    current_correction.status = CorrectionStatus.FAILED
                                    current_correction.error_message = "任务执行失败或被撤销"
                                    logger.info(f"更新作文 {essay.id} 和批改 {correction.id} 的状态为FAILED")
                                else:
                                    # 未知状态，重置为待处理
                                    current_essay.status = EssayStatus.PENDING
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='修复当前卡住的任务')
    parser.add_argument('--apply', action='store_true', help='应用修复（默认为干运行模式）')
    args = parser.parse_args()
    
    apply_fix = args.apply
    mode = "修复模式" if apply_fix else "干运行模式"
    logger.info(f"开始修复当前卡住的任务 ({mode})")
    
    result = fix_inconsistent_statuses(apply_fix=apply_fix)
    ...
    if not apply_fix and (result['inconsistent_count'] > 0 or result['stale_count'] > 0):
