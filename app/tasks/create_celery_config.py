...
def create_celery_config():
    ...
    try:
        ...
        celery_config_path = os.path.join(script_dir, 'celery_config.py')
        
        logger.info(f"开始创建Celery配置文件: {celery_config_path}")
        
        celery_config_content = '''#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
# Prometheus监控配置 
prometheus_metrics_enabled = config.TASK_CONFIG.get('prometheus_metrics_enabled', False)
...
if prometheus_metrics_enabled:
    try:
        ...
        def start_prometheus_server():
            ...
            try:
                prometheus_port = config.TASK_CONFIG.get('prometheus_port', 9090)
...
'''
        ...
        # 写入配置文件
        with open(celery_config_path, 'w', encoding='utf-8') as f:
            f.write(celery_config_content)
        
        logger.info(f"Celery配置文件已成功创建: {celery_config_path}")
        return True
    except Exception as e:
        logger.error(f"创建Celery配置文件失败: {str(e)}")
        return False
